...
//    func refreshAllApps(identifier: String = UUID().uuidString, completionHandler: @escaping (Result<[String: Result<InstalledApp, Error>], Error>) -> Void)
//    {
//        DatabaseManager.shared.persistentContainer.performBackgroundTask { (context) in
//            let installedApps = InstalledApp.fetchAppsForBackgroundRefresh(in: context)
...
//                context.perform {
//                    guard !ServerManager.shared.discoveredServers.isEmpty else { return completionHandler(.failure(RefreshError.noInstalledApps)) }
//
...
private extension AppDelegate
{
    ...
    func refreshApps(completionHandler: @escaping (Result<[String: Result<InstalledApp, Error>], Error>) -> Void)
    {
        DatabaseManager.shared.persistentContainer.performBackgroundTask { (context) in
            let installedApps = InstalledApp.fetchAppsForBackgroundRefresh(in: context)
            AppManager.shared.backgroundRefresh(installedApps, completionHandler: completionHandler)
        }
    }
