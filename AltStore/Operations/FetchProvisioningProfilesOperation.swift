    ...
    func prepareProvisioningProfile(for app: ALTApplication, parentApp: ALTApplication?, team: ALTTeam, session: ALTAppleAPISession, completionHandler: @escaping (Result<ALTProvisioningProfile, Error>) -> Void)
    {
        DatabaseManager.shared.persistentContainer.performBackgroundTask { (context) in
            ...
            else
            {
                ...
                if app.bundleIdentifier.hasPrefix(StoreApp.altstoreAppID) || StoreApp.alternativeAltStoreAppIDs.contains(app.bundleIdentifier)
                {
                    // Use legacy bundle ID format for AltStore (and its extensions).
                    updatedParentBundleID = "com.\(team.identifier).\(app.bundleIdentifier)"
                }
                else
                {
